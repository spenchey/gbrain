...
Filing note: EIIRP files across the full canonical set. The actual destination
per page is decided by brain-taxonomist consulting the active schema pack via
`gbrain schema show --json`, so custom schema packs may route more broadly.
