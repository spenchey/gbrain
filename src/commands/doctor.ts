...
export async function buildChecks(
  ...
  try {
    ...
    // A successful restart begins a new health epoch. Older crash and
    // max-crash events remain in the audit trail, but must not poison the
    // currently running supervisor's health verdict.
    ...
    if (lastStarted && engine) {
      ...
      if (localPid !== null) {
        try {
          ...
          // A stale or inaccessible pid cannot prove hostname equivalence.
        }
      }
