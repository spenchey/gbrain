...
export async function embed(texts: string[], opts?: EmbedOpts): Promise<Float32Array[]> {
  ...
  try {
    for (const batch of batches) {
      // Local Ollama branch (custom commit 1a679a31 "Support env-configured Ollama
      // embeddings") — Ollama uses a non-AI-SDK shape, so it goes through its own
      // sub-batch helper. Everything else uses the gateway's AI SDK path.
      const result = recipe.id === 'ollama'
        ? await embedOllamaSubBatch(batch, cfg, recipe, modelId, expected, opts)
        : await embedSubBatch(batch, model, providerOpts, expected, recipe, modelId, opts);
...
async function embedOllamaSubBatch(
  ...
  const baseUrl = cfg.base_urls?.[recipe.id] ?? recipe.base_url_default;
  if (!baseUrl) throw new AIConfigError(`Ollama embeddings require a base URL.`, recipe.setup_hint);
