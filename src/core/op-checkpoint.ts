...
export async function recordCompleted(
  ...
  // exactly as before. Real Postgres + postgres.js binds JSON.stringify() as
  // a JSON string, so send text[] and convert it server-side with to_jsonb()
  // to preserve the array shape. Sync uses `appendCompleted` (below), never this.
